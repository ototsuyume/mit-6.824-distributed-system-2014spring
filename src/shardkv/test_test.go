...
func TestMove(t *testing.T) {
	...
	for i := 0; i < shardmaster.NShards; i++ {
		if ck.Get(string('0'+i)) != string('0'+i) {
			t.Fatalf("missing key/value %d %s", i, string('0'+i))
		}
	}
...
func TestLimp(t *testing.T) {
	...
	vals := make([]string, len(keys))
	for i := 0; i < len(keys); i++ {
		keys[i] = strconv.Itoa(i) //strconv.Itoa(rand.Int())
		vals[i] = strconv.Itoa(i) //strconv.Itoa(rand.Int())
		ck.Put(keys[i], vals[i])
	}
	...
	for i := 0; i < npara; i++ {
		...
		go func(me int) {
			...
			for iters := 0; iters < 3; iters++ {
				...
				if v != last {
					ok = false
					t.Fatalf("PutHash(%v) expected %v %v got %v\n", key, last, nv, v)
				}
