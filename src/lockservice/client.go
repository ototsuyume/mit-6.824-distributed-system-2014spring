...
func (ck *Clerk) Lock(lockname string) bool {
	...
	args := &LockArgs{lockname, fmt.Sprintf("%d_%d", time.Now().UnixNano(), nrand())}
	reply := LockReply{false}
	// send an RPC request, wait for the reply.
	ok := call(ck.servers[0], "LockServer.Lock", args, &reply)
	if ok == false {
		call(ck.servers[1], "LockServer.Lock", args, &reply)
	}
...
func (ck *Clerk) Unlock(lockname string) bool {
	...
	args := &UnlockArgs{lockname, fmt.Sprintf("%d_%d", time.Now().UnixNano(), nrand())}
	reply := UnlockReply{false}
	ok := call(ck.servers[0], "LockServer.Unlock", args, &reply)
	if ok == false {
		call(ck.servers[1], "LockServer.Unlock", args, &reply)
	}
